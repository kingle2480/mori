...
# generate_appcast may output appcast.xml or use the existing filename.
# Find whichever XML file it produced.

APPCAST_OUT=""
for candidate in "$UPDATES_DIR/appcast.xml" "$UPDATES_DIR/mori-appcast.xml"; do
  if [[ -f "$candidate" ]]; then
    APPCAST_OUT="$candidate"
    break
  fi
done

# Fallback: pick any XML file
if [[ -z "$APPCAST_OUT" ]]; then
  APPCAST_OUT=$(find "$UPDATES_DIR" -maxdepth 1 -name "*.xml" -type f | head -1)
fi

if [[ -n "$APPCAST_OUT" && -f "$APPCAST_OUT" ]]; then
  cp "$APPCAST_OUT" ./mori-appcast.xml
  echo "Generated mori-appcast.xml successfully (from $(basename "$APPCAST_OUT"))."
else
  echo "Error: no appcast XML was generated by generate_appcast" >&2
  echo "Contents of updates dir:"
  ls -la "$UPDATES_DIR"
  exit 1
fi
